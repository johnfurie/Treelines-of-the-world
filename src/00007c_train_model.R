...
index = CAST::CreateSpacetimeFolds(dat,spacevar = "train", k = 2)


# train control for rf model - with LLO-CV based on forest sections
tC = caret::trainControl(method = "cv", number =  2,  classProbs = TRUE, index = index$index, indexOut = index$indexOut )
