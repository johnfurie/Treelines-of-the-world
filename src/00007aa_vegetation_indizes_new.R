...
#write data
writeRaster(ind_stk, file.path(envrmt$path_002_processed,"veg_ind_e1.tif"),format="GTiff",overwrite=TRUE)
...
# write
writeRaster(all_stk, file.path(envrmt$path_002_processed,"veg_ind_new3.tif"),format="GTiff",overwrite=TRUE)
all_stk       <- raster::stack(file.path(envrmt$path_002_processed, "veg_ind3_new.tif"))
...
fil <- filter_Stk(all_stk,fLS="all",sizes=c(31),layernames=ln)
#names(fil)
